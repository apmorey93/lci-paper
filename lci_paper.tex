...
\end{equation}
The raw cost $C/T(u)$ \emph{decreases} in $u$ (a fixed resource bill is spread over more throughput), while the quality markup $1/\phi(u)$ \emph{increases} in $u$ (p95 latency rises near saturation, so $\lambda(\ell)$ and hence $\phi$ fall). Their product is U-shaped with an interior cost-minimizing utilization $u^\ast$; near $u_{\max}$ the quality term dominates and $\LCI(u)$ rises convexly (Property~C; Appendix~\ref{app:queue}). Figure~\ref{fig:convex} shows the simulated relationship; the curve is \emph{measured} from the validated discrete-event simulator (Section~\ref{sec:results}), not posited.
...
\caption{Throughput (normalized, increasing) and cost per quality-adjusted task $\LCI(u)$ vs.\ utilization, from the validated GI/G/$k$--PS+batching simulator (QA family, 2025 inputs). Cost-spreading lowers $\LCI$ at light load; QoS erosion raises it near saturation. The minimum defines $u^\ast\approx0.88$.}
\label{fig:convex}
\end{figure}
...
\caption{Estimated IPD (chain Fisher, equal weights), base $=100$ at 2023Q3, with Laspeyres and Paasche bounds. The index falls to $\approx$48 by 2025Q2 (Table~\ref{tab:ipd}), a $\sim$52\% decline in the quality-adjusted cost of intelligence, driven by sourced GPU price cuts, faster decoding, and benchmark-accuracy gains.}
\label{fig:ipd}
\end{figure}
...
Poisson arrivals, exponential service, QoS weights
$(\eta_a,\eta_\ell,\eta_q,\eta_s)=(0.5,0.2,0.2,0.1)$, softness $\tau{=}1500$\,ms;
operational $q{=}0.999$, $s{=}0.98$ are documented assumptions (no sourced
per-model safety panel). Inputs: AWS p4d/p5 on-demand list prices (A100 \& H100,
...
(Figure~\ref{fig:lcicurve}). The reported LCI for each (date, family) is the
minimum over available GPU options—the cost frontier.

\begin{figure}[t]
\centering
\includegraphics[width=0.58\textwidth]{results/figures/fig_LCI_curve.pdf}
\caption{Estimated $\LCI(u)$ by task family (2025 inputs); markers denote the cost-minimizing utilization $u^\ast$.}
\label{fig:lcicurve}
\end{figure}
...
\end{tabular}
\begin{tablenotes}[flushleft]
\item \emph{Note:} Self-hosted Llama-70B frontier; latency/throughput simulated, prices/accuracy sourced. Frontier GPU is A100-class throughout (lower \$/token than H100 at these assumptions). Full per-row detail (GPU, $a$, p95, $\phi$, PUI, wedge) in \texttt{results/tables/lci\_by\_family.csv}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
and Figure~\ref{fig:ipd}: a fall from 100 to $\approx$48 over 2023Q3--2025Q2.
...
$\text{PUI}/\LCI$ falls from $\sim$40--50$\times$ in 2023 (GPT-4-era pricing)
...
\caption{Market price (PUI, public API list prices) vs.\ the LCI cost frontier over time (log scale). The wedge compresses from $\sim$40--50$\times$ to below $1\times$.}
\label{fig:wedge}
\end{figure}
